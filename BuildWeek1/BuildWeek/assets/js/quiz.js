...
window.onload = function () {
  ...
  function checkAnswer(selected) {
    ...
    allButtons.forEach((button) => {
      if (button.textContent === selected) {
        //selezione delle risposte > background del bottone
        button.style.backgroundColor = "#900080";
        button.style.color = "#fff";
        button.style.border = "none";
      } else {
    ...
    if (selected === questions[currentQuestion].correct_answer) {
      //se la risposta selezionata è CORRETTA, incrementa di 1 lo score
      score++;
    }
  }
  ...
  document.getElementById("nextButton").addEventListener("click", () => {
    if (currentQuestion === -1) {
      //se la domanda non esiste (-1) fai partire il quiz
      startQuiz();
    } else {
      if (!userHasAnswered) {
    ...
    if (currentQuestion < questions.length - 1) {
      //se l'indice della domanda è minore della lunghezza dell'array vai avanti
      currentQuestion++;
      displayQuestion();
      startTimer(41);
  ...
  `;
    ...
    passStatus.textContent =
      percentageCorrect >= 60
        ...
        : "Oh no! you failed the exam ç_ç"; //operatore ternario
  ...
  function startTimer(duration) {
    ...
    timerInterval = setInterval(() => {
      // Avviamo un nuovo timer che si ripete ogni secondo
      ...
      if (elapsed === duration) {
        // Se il tempo trascorso è uguale alla durata totale, fermiamo il timer e passiamo alla prossima domanda
        clearInterval(timerInterval);
        goToNextQuestion();
      }
    }, 1000);
